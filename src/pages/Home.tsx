  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative hero-gradient overflow-hidden min-h-[65vh] md:min-h-[70vh] flex items-center">
        ...
        <div className="container mx-auto px-4 py-20 md:py-24 relative z-10">
          ...
          >
            ...
            <motion.p variants={fadeInUp} className={`text-base sm:text-lg text-muted-foreground mb-10 max-w-2xl mx-auto leading-relaxed ${language === 'bn' ? 'font-bangla' : ''}`}>
              {t(
          ...
          >
            {features.map((feature, index) => (
              <motion.div
                key={index}
                variants={fadeInUp}
                className="flex items-start gap-4 p-5 sm:p-6 rounded-xl bg-card border border-border hover:shadow-md hover:border-primary/20 transition-all duration-300"
              >
                <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0">
                  <feature.icon className="w-6 h-6 text-primary" />
                </div>
                <div>
                  <h3 className={`font-bold text-foreground text-lg ${language === 'bn' ? 'font-bangla' : ''}`}>
      ...
      <section className="py-12 md:py-16 bg-secondary/30">
        <div className="container mx-auto px-4">
          <motion.div
            className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-5 lg:gap-8"
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, margin: '-60px' }}
            variants={staggerContainer}
          >
            {stats.map((stat, index) => (
              <motion.div
                key={index}
                variants={fadeInUp}
                className="text-center p-5 sm:p-6 rounded-xl bg-card border border-border hover:shadow-md transition-all duration-300"
              >
      ...
      {/* Doctors Grid */}
      <section className="py-12 md:py-16 bg-background" id="doctors">
        <div className="container mx-auto px-4">
          <motion.div
          ...
          {filteredDoctors.length > 0 ? (
            <motion.div
              className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 lg:gap-8"
              initial="hidden"
              whileInView="visible"
              viewport={{ once: true, margin: '-60px' }}
              variants={staggerContainer}
        ...
        <div className="container mx-auto px-4">
          ...
          >
            ...
            <motion.a
              variants={fadeInUp}
              href="#doctors"
              className={`btn-primary-gradient px-8 sm:px-10 py-3 sm:py-4 rounded-full text-base sm:text-lg font-semibold inline-block ${language === 'bn' ? 'font-bangla' : ''}`}
            >
